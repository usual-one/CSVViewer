...
void MainWindow::showCalculationResults() {
    ...
    model->setItem(0, 1, new QStandardItem(QString::number(response.metrics[0])));
    model->setItem(1, 1, new QStandardItem(QString::number(response.metrics[1])));
    model->setItem(2, 1, new QStandardItem(QString::number(response.metrics[2])));
